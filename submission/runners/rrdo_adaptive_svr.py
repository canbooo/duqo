...
def direct_rrdo(objectives, constraints, trainer, trainer_args, start_samples,
    ...
    res = lolhr4rrdo(problem, lower, upper, opter, trainer, step_size,
                     max_samples, start_doe=start_samples,
    ...
    if not res_key.endswith(f"_adapt_svr_{start_samples}_{step_size}_{max_samples}_pred_res.pkl"):
        res_key += f"_adapt_svr_{start_samples}_{step_size}_{max_samples}_pred_res.pkl"
...
def main(exname, save_dir=".", force_pop_size=None, force_opt_iters=None):
    ...
    return direct_rrdo(obj_fun, con_fun, model_trainer, funs,
                       ...
                       pop_size=2 * popsize, max_gens=2 * maxgens, opt_inps=opt_inps, res_key=res_key)
