...
Gu et al., A comparative study on multiobjective reliable and robust optimization for crashworthiness design of vehicle
structure (2013)
Gu et al., Reliability-based robust assessment for multiobjective optimization design of improving occupant restraint
...
def select_candidates(front, archive, n_points):
    """
    Select knee point as proposed in
    Gu et al. 2013 A comparative study on multiobjective reliable and robust optimization for crashworthiness
    design of vehicle structure
    Gu et al. 2014 Reliability-based robust assessment for multiobjective optimization design of improving occupant
    ...
    if front.shape[0] < 2:
        return archive
    scaler = MinMaxScaler()
    scaled = scaler.fit_transform(front.copy())
    best_id = np.argmin(np.linalg.norm(scaled, axis=1))
    points = archive[[best_id], :]
    remaining = min(archive.shape[0], n_points) - 1
    if remaining:
        choices = [k for k in range(archive.shape[0]) if k != best_id]
        choices = np.random.choice(np.array(choices, dtype=int), size=remaining, replace=False)
    return np.append(points, archive[choices.tolist()], axis=0)


def direct_rrdo(objectives, constraints, trainer_args, start_samples, step_size, max_samples, model_objectives,
                model_constraints, num_obj, num_con, n_inp_total, lower, upper, margs, sto_inps=None, opt_inps=None,
                scale_objs=False, ra_methods=None, obj_arg=None, con_arg=None, sto_obj_inds: list = None,
                sto_con_inds: list = None, obj_wgt=1.96, base_doe=True, target_fail_prob=None, pop_size=100,
                max_gens=100, punish_factor=100, pareto_size=1000, verbose=0, start_gen=None, res_key=None):
    ...
    cur_doe = make_doe(start_samples, lower_bound=lower_doe, upper_bound=upper_doe)
    ...
    opter = InspyredOptimizer(obj_con, lower, upper, num_obj, method="NSGA", verbose=verbose, scale_objs=scale_objs)
    res = opter.optimize(pop_size=pop_size, max_gens=max_gens,
                         punish_factor=punish_factor,
                         pareto_size=pareto_size, verbose=verbose,
                         start_gen=start_gen)
    while cur_doe.shape[0] < max_samples:
        cands = np.array([np.array(r.candidate) for r in res])
        front, cons = obj_con(cands)
        valid = np.min(cons, axis=1) >= 0
        cands = cands[valid]
        front = front[valid]
        ...
        models = model_trainer(np.unique(cur_doe, axis=0), *trainer_args)
        full_space.obj_arg[-1] = models
        full_space.con_arg[-1] = models
        res = opter.optimize(pop_size=pop_size, max_gens=max_gens,
                             punish_factor=punish_factor,
                             pareto_size=pareto_size, verbose=verbose,
                             start_gen=start_gen)
    ...
    if not res_key.endswith(f"_adapt_mean_gp_{start_samples}_{step_size}_{max_samples}_pred_res.pkl"):
        res_key += f"_adapt_mean_gp_{start_samples}_{step_size}_{max_samples}_pred_res.pkl"
